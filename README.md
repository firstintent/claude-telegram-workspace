...
```bash
# 1. clone 到临时位置（不是你的项目）
git clone --depth 1 https://github.com/firstintent/claude-telegram-workspace.git /tmp/cct-tpl

# 2. 拷 skills 到已有项目
cd /path/to/your-project
cp -r /tmp/cct-tpl/.claude/skills/{telegram-configure,telegram-access,cct-slash,cct-snapshot} .claude/skills/
mkdir -p .claude/channels/telegram/approved
touch .claude/channels/telegram/.gitkeep .claude/channels/telegram/approved/.gitkeep

# 3. settings.local.json 与 .gitignore 手工合并（对照 /tmp/cct-tpl/install.sh）

# 4. 删 clone，后续启动/接入/配对同方式一第 3 步起
rm -rf /tmp/cct-tpl
```
...
机器
 ...
 │    │    ├── skills/                     ← 项目级 skills
 │    │    │    ├── telegram-configure/    ← Bot 接入（覆写官方 telegram:configure）
 │    │    │    ├── telegram-access/       ← 用户配对（覆写官方 telegram:access）
...
| `.claude/skills/telegram-configure/` | Bot 接入 skill（覆写官方 telegram:configure） |
| `.claude/skills/telegram-access/` | 用户配对 skill（覆写官方 telegram:access） |
