...
- **多工作区独立 Bot** — 每个工作区绑定独立的 Bot Token，`TELEGRAM_STATE_DIR` 隔离，同一台机器并行运行互不干扰；解决了 Claude Code 官方 Telegram 插件全局单 Bot 的限制，是当前的最佳实践
- **Telegram 内执行 Claude Code 命令** — 通过 `cct-slash` skill，可在 Telegram 消息中向任意 tmux session 发送 `/clear`、`/compact`、`/memory` 等 Claude Code slash 命令，无需切换终端
- **CLAUDE.md 模板（可选）** — 内置团队协作规范模板，开箱即用；可按需裁剪或替换为自己的配置
...
# 然后告诉 Claude：帮我接入 Telegram，token 是 <token-a>

# 工作区 B
cd /path/to/workspace-b && tmux new -s ws-b
# tmux 内：claude --channels plugin:telegram@claude-plugins-official
# 然后告诉 Claude：帮我接入 Telegram，token 是 <token-b>
```
